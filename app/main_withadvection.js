...
const velocity = function(x) {

	if (x < 10) {
		return Math.round(0.02 * x);;
	}

	if (x >= 20) {	
		return -Math.round(0.1 * x);
	}

	return Math.round(0.2 * x);
}
...
const advection_cld = function (i, j, k) {
   ...
   if (i - velocity(k) > 0) {
   	pt.cld = get_Voxel(i - velocity(k), j, k).next_cld;
   }
   else {
   	pt.cld = 0;
   }  
...
const advection_hum = function (i, j, k) {
   ...
   if (i - velocity(k) > 0) {
   	pt.hum = get_Voxel(i - velocity(k), j, k).next_hum;
   }
   else {
   	pt.hum = 0;
   }  
...
const advection_act = function (i, j, k) {
   ...
   if (i - velocity(k) > 0) {
   	pt.act = get_Voxel(i - velocity(k), j, k).next_act;
   }
   else {
   	pt.act = 0;
   }  
...
const update_all = function () {
   ...
   for (let i = 0; i < cloud_parts.length; i++) {
      ...
      if (pt.cld == false) {
         pt.part.material.opacity = 0.0;
         pt.part.material.color.setHex(0xB020C1);
      } else {
         pt.part.material.opacity = 0.02;
         pt.part.material.color.setHex(0xd3d3d3);
